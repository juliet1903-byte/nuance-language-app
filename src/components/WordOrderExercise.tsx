...
const WordOrderExercise = ({ instruction, items, onComplete }: WordOrderExerciseProps) => {
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0 }}
    >
      <h2 className="text-xl font-medium mb-2">Word Order</h2>
      ...
      <div className="bg-card rounded-xl p-4 shadow-sm mb-4 min-h-[60px]">
        <div className="flex flex-wrap gap-2">
          ...
          {selectedWords.length === 0 && (
            <p className="text-xs text-muted-foreground/40 py-2">Tap words below to build the sentence…</p>
          )}
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        {availableWords.map((word, i) => (
          <motion.button
            ...
            disabled={checked}
            className="text-xs px-4 py-2 rounded-full font-medium shadow-sm bg-card text-foreground active:scale-95 transition-all"
            whileTap={{ scale: 0.9 }}
          >
            {word}
          </motion.button>
